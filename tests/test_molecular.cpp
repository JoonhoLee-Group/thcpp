...
TEST_CASE("test_mol_thc", "[molecular]")
{
  ...
  DistributedMatrix::Matrix<double> orbs("data/eri_thc.h5", "etaPm", BH.Root, true, false);
  if (BH.rank == 0) {
  ...
  DistributedMatrix::Matrix<double> df_ints("data/eri_thc.h5", "eri_df", BH.Root, true, false);
  if (BH.rank == 0) {
    // Need to transpose data for scalapack
...
TEST_CASE("test_find_pseudo_inverse", "[molecular]")
{
  ...
  std::string input_file = "data/eri_thc.h5";
...
{
  generate_qmcpack_molecular_thc("data/eri_thc.h5", "hamil_test.h5", true, true);
}
