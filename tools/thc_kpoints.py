...
cell, mf, hcore, fock, AORot, kpoints, ehf_kpts, uhf = init_from_chkfile('scf.kpoint.2x2x2.ngs.12.dump')
nb = hcore.shape[-1]
...
for iq, Q in enumerate(kpoints):
    ...
    # pl.yscale('log')
    # pl.show()
    L = modified_cholesky(MUV, 1e-6, verbose=True, cmax=100).T
    ...
    with h5py.File('integrals.h5', 'a') as fh5:
        fh5['Hamiltonian/KPTHC/L%i'%iq] = L.view(numpy.float64).reshape(L.shape+(2,))
...
with h5py.File('integrals.h5', 'a') as fh5:
    ...
    fh5['Hamiltonian/NMOPerKP'] = numpy.array([nmo]*8)
